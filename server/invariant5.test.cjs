...
test('INV5: a non-curated fact is verified ONLY via a recorded acceptance event — faked fields do not count', () => {
  ...
  assert.strictEqual(isVerifiedFact({ id: 'c', acceptance: { id: 'acceptance_x' } }), true);
...
test('INV5: both mint paths attach the acceptance event with reviewed wording AND reviewed attribution', async () => {
  ...
  const m = await engine.personMint({ store, llm: null, text: 'Ran the vendor selection end to end', attribution: { type: 'skill' } });
...
test('INV5: every refusal path mints NOTHING (nonce, attribution, defective, ceiling)', async () => {
  ...
  const r = await engine.personMint({ store, llm: null, text: 'Legit line', attribution: { type: 'skill' } });
  assert.strictEqual(r.outcome, 'refused');
