...
- **LinkedIn.** Banned: the LinkedIn API, software posting on the person's behalf, and scraping people or profiles (automation and scraping are against LinkedIn's terms, and a restricted account mid-job-search is a catastrophe for a jobseeker). Allowed and in scope: everything up to the send button - what to post, where, when, how, and the follow-up. The person's own finger does the posting (E1).
