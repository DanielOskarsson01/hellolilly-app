...
- **Remove the old localStorage decision path — no dead fallback.** `jobStore.js`'s decision functions (`acceptJob`/`removeJob`/`getAcceptedJobs`/`removeAcceptedJob`/`getRemovedJobIds`) and their callers (`jobResultsList.jsx` handlers) are REMOVED. A silent localStorage decision fallback recreates the exact row-vs-layover split this build eliminates. Grep every usage; migrate reads to `useJobs`/`listJobs`; the build report confirms removal. (Non-decision jobStore state — saved searches, latest search, active case — may stay if still used.)
...
**Deferred/known gaps (flag, do not build):** CSV ingest wiring, custom-source URL ingestion, bulk approve/reject, decided-history pagination beyond last-10, reject-reason read-back surface, conflicting-evidence reconciliation. Matchanalys restyle is a separate wave.
...
- [ ] **Step 5: Remove the localStorage decision path (no dead fallback).** `grep -rn "acceptJob\|removeJob\|getAcceptedJobs\|removeAcceptedJob\|getRemovedJobIds" src/` — remove these functions from `jobStore.js` and every caller (the old `jobResultsList.jsx` decision handlers; `jobResultsList.jsx` is orphaned by the rewrite — delete it if nothing else imports it). If a wired screen (e.g. `home.jsx`) reads accepted/removed jobs from localStorage, SURFACE it and migrate that read to `listJobs`/`useJobs` (decisions now live in the backend). Re-grep must return zero decision-function references outside `useJobs`/`caseApi`.
...
- [ ] **Step 7: Commit** — `git add src/screens/jobSearch.jsx src/utils/jobStore.js && git commit -m "feat(jobbsok): one screen on grid templates — results are the approval surface; localStorage decision path removed"`
