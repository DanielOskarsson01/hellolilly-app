...
**Case Record (jobseeker-facing)** - one chronological reading view over everything the case already holds. Tier: NOW. The coach-facing variant waits for identity.
...
**Coach tools (Coach Network, Coach Review, coach-facing Case Record, Transition View)** - the human-anchoring surfaces. All frontends buildable now with the fixture coach cast; each flips real by changing data, not code when a pilot coach replaces a placeholder. Tier: SIM - and this is the multi-user identity trigger point.
