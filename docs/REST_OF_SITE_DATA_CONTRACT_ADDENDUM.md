...
- **Writable:** user — create, `question`, withdraw. AI — nothing (responses are a real human's words; no model authors a coach voice). System — labels, timestamps, `status` transitions, and — fed by the coach-channel adapter — the `dispatch` record on send and each inbound `responses[]` entry on reply.
