...
- **Reach architecture (per D21):** the real version runs a **local vault** on the coach's own machine (her exported + optionally enriched LinkedIn network, encrypted, never sent to HelloLilly); matching is an **edge judge** on the coach's side (vault text enters it inside the D12 Rule 2 injection envelope); only **binary verdicts (JA/NEJ) plus opaque row IDs** ever cross to HelloLilly — no names, no rows, no descriptive text. On JA the **owner coach resolves the IDs locally** to her real rows and makes the intro herself; **everyone else sees only a uniform sentence** ("en förfrågan har gått till en coach" / "ingen träff just nu"). Tier unchanged (demo) until Path A + the §2 governance review; central development runs on **synthetic vaults only**.
...
- **Reach architecture (per D21):** when reach goes real, the directory never exposes network rows — each coach's network lives as a **local vault** on her own machine (encrypted, never sent to HelloLilly); matching is an **edge judge** on the coach's side (vault text enveloped per D12 Rule 2); only **binary verdicts (JA/NEJ) plus opaque row IDs** transit to HelloLilly — no names, no rows, no descriptive text. The **owner coach resolves the IDs locally** to her real rows and makes the intro herself; **everyone else sees only a uniform sentence** ("en förfrågan har gått till en coach" / "ingen träff just nu"). Tier unchanged (demo) until Path A + the §2 governance review; central development runs on **synthetic vaults only**.
...
**Parked as T5 panels:** Live Support, the native app, tender/investment/leadership radar sources. **Parked invisibly:** fine-tuned models. **Refused on principle (no panel — not-ever gets nothing):** LinkedIn API automation, automated outreach, scraping people *(amended by D21, 2026-07-16: coach's own network via coach's own tooling - see D21)*, synthetic faces/identity edits, pre-vote poll results, fake analytics presented as computed.
